...
public class ToplevelService
{
    ...
    {
        if (sender is null)
            throw new InvalidOperationException("The ToplevelService can only be registered on a Visual");

        // Unregister any old registered context
        if (e.OldValue != null) RegistrationMapper.Remove(e.OldValue);

        // Register any new context (use indexer to avoid duplicate key exception)
        if (e.NewValue != null) RegistrationMapper[e.NewValue] = sender;
    }

    /// <summary>
    ///     This property handles the registration of Pages and ViewModel
    /// </summary>
    public static readonly AttachedProperty<object?> RegisterProperty =
        AvaloniaProperty.RegisterAttached<ToplevelService, Visual, object?>(
            "Register");
