...
public class RenderQueueApiManager : IDisposable
{
    ...
    private bool IsApiEnabled { get; set; } = true;

    private int ApiPort { get; set; } = 8325;
    ...
    public async Task StartApiAsync()
    {
        ...
        if (_apiService is { IsRunning: true })
        {
        ...
        // 如果服务存在但未运行，先清理
        if (_apiService is { IsRunning: false })
        {
            ...
            try
            {
                ...
                if (_apiService is IDisposable disposable) disposable.Dispose();
            }
            catch (Exception ex)
            {
    ...
    public async Task SetApiConfigAsync(bool enabled, int port)
    {
        ...
        // 如果正在运行，先停止
        if (wasRunning) await StopApiAsync();

        IsApiEnabled = enabled;
        ApiPort = port;

        // 如果启用API，则启动服务
        if (enabled) await StartApiAsync();
    }
    ...
    public void Dispose()
    {
        ...
        if (_apiService is IDisposable disposableApiService) disposableApiService.Dispose();
