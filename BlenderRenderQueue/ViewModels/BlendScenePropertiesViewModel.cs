...
public partial class BlendScenePropertiesViewModel : ViewModelBase
{
	...
	partial void OnActiveSceneNameChanged(string value)
	{
		...
		SceneProperties = ActiveSceneProperties;
	...
	/// 获取所有场景名称列表
	/// </summary>
	public List<string> SceneNames => AllScenes.Keys.ToList();
	...
	public bool ShowEmptyState
	{
		get
		{
			...
			Console.WriteLine($"[BlendScenePropertiesViewModel] ShowEmptyState calculated: {result} (IsLoaded: {ActiveSceneProperties.IsLoaded}, IsLoading: {IsLoading})");
			return result;
		}
	}
	...
	public async Task LoadPropertiesAsync(BasePythonProcessService process, string blendFilePath, CancellationToken cancellationToken = default)
	{
		...
		Console.WriteLine($"[BlendScenePropertiesViewModel] Initial state - IsLoading: {IsLoading}, IsLoaded: {ActiveSceneProperties.IsLoaded}, ShowEmptyState: {ShowEmptyState}");
		...
		try
		{
			...
			var (activeScene, sceneData) = await queryService.GetAllFilePropertiesAsync(process, blendFilePath, cancellationToken);
			...
			Console.WriteLine($"[BlendScenePropertiesViewModel] Properties loaded successfully - ActiveScene: {ActiveSceneName}, ScenesCount: {AllScenes.Count}, IsLoaded: {ActiveSceneProperties.IsLoaded}");
			...
			Console.WriteLine($"[BlendScenePropertiesViewModel] After loading - IsLoading: {IsLoading}, IsLoaded: {ActiveSceneProperties.IsLoaded}, ShowEmptyState: {ShowEmptyState}");
		}
		catch (Exception ex)
		{
		...
		finally
		{
			IsLoading = false;
			OnPropertyChanged(nameof(ShowEmptyState));
			Console.WriteLine($"[BlendScenePropertiesViewModel] Finally block - IsLoading: {IsLoading}, IsLoaded: {ActiveSceneProperties.IsLoaded}, ShowEmptyState: {ShowEmptyState}");
		}
	}
	...
	public bool CanOpenFramePathDirectory => !string.IsNullOrEmpty(ActiveSceneProperties.FramePath) && 
	                                         !string.IsNullOrEmpty(Path.GetDirectoryName(ActiveSceneProperties.FramePath));
}
