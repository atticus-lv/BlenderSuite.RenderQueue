...
def find_app_launch_target(app_launch_path: str = "") -> str | None:
    ...
    explicit_path = os.environ.get("BSRQ_APP_PATH")
    if explicit_path and os.path.exists(explicit_path):
        return explicit_path

    executable_names = ["BlenderSuite.RenderQueue.exe", "BlenderSuite.RenderQueue"]
    ...
    if sys.platform == "win32":
        for env_var in ("ProgramFiles", "ProgramFiles(x86)"):
            ...
            if base:
                candidate = Path(base) / "BlenderSuite.RenderQueue" / "BlenderSuite.RenderQueue.exe"
                if candidate.is_file():
                    return str(candidate)
    elif sys.platform == "darwin":
        for candidate in (
            Path("/Applications/BlenderSuite.RenderQueue.app"),
            Path.home() / "Applications" / "BlenderSuite.RenderQueue.app",
        ):
            if candidate.exists():
                return str(candidate)
...
def _should_auto_start_queue() -> bool:
    env_value = os.environ.get("BSRQ_AUTO_START_QUEUE", "").strip().lower()
...
def wait_for_endpoint(report_callback=None) -> dict:
    ...
    if report_callback is not None:
        report_callback({"WARNING"}, f"Failed to connect to Blender Suite Render Queue after startup: {last_error}")
...
def ensure_endpoint(auto_start_app: bool, app_launch_path: str = "", report_callback=None) -> dict:
    ...
    if not auto_start_app:
        raise RuntimeError("Blender Suite Render Queue is not running and auto-start is disabled.")

    launch_target = find_app_launch_target(app_launch_path)
    if not launch_target:
        raise RuntimeError("Blender Suite Render Queue is not running and no launch target was found.")
...
def submit_task_payload(
    ...
    *,
    ...
    if not blend_file_path:
        raise RuntimeError("Save the .blend file before submitting it to Blender Suite Render Queue.")
