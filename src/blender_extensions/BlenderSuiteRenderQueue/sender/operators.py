...
class BSRQ_OT_submit_scene(Operator):
    bl_idname = "blendersuite_renderqueue.submit_scene"
    bl_label = "Submit Scene to Queue"
    bl_description = "Submit current scene to Blender Suite Render Queue"
    ...
    def execute(self, context):
        if not bpy.data.filepath:
            self.report({"ERROR"}, "Save the .blend file before submitting it to Blender Suite Render Queue.")
            return {"CANCELLED"}
        ...
        self.report({"INFO"}, "Submitting scene to Blender Suite Render Queue...")
