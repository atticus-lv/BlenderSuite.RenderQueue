...
public partial class App : Application
{
    ...
    public override void OnFrameworkInitializationCompleted()
    {
        if (ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            ...
            {
                _renderLogService?.Write(RenderLogLevel.Error, RenderLogScope.System, $"Failed to start local submission host: {ex.Message}", source: nameof(App));
    ...
    private Task StopLocalSubmissionHostAsync()
    {
        lock (_localSubmissionHostStopLock)
        {
            ...
            _localSubmissionHostStopTask = Task.Run(async () =>
            {
                ...
                {
                    _renderLogService?.Write(RenderLogLevel.Error, RenderLogScope.System, $"Failed to stop local submission host: {ex.Message}", source: nameof(App));
                    _renderLogService?.Write(
                        RenderLogLevel.Warning,
                        RenderLogScope.Submission,
                ...
                finally
                {
                    ...
                    {
                        _renderLogService?.Write(RenderLogLevel.Error, RenderLogScope.System, $"Failed to dispose local submission host: {ex.Message}", source: nameof(App));
                    }
                }
            });
