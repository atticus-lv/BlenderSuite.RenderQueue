...
{
    private const string ExtensionId = "BlenderSuiteRenderQueue";
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (sourceDirectory != null)
        {
            ...
            await RunBlenderCommandAsync(
                ...
                $"--command extension build --source-dir {Quote(sourceDirectory)} --output-dir {Quote(outputDirectory)}",
                cancellationToken);
    ...
    {
        var packagedSourceDirectory = Path.Combine(AppContext.BaseDirectory, "Resources", "BlenderExtensions", ExtensionId);
        if (Directory.Exists(packagedSourceDirectory))
        {
            return packagedSourceDirectory;
        }

        var current = new DirectoryInfo(AppContext.BaseDirectory);
        while (current != null)
        {
            var candidate = Path.Combine(current.FullName, "src", "blender_extensions", ExtensionId);
            if (Directory.Exists(candidate))
            {
                return candidate;
            }

            candidate = Path.Combine(current.FullName, "scripts", "BlenderExtensions", ExtensionId);
            if (Directory.Exists(candidate))
            {
                return candidate;
            }
    ...
    private async Task RemovePackageAsync(string blenderExecutablePath, string packageId, CancellationToken cancellationToken)
    {
        await RunBlenderCommandAsync(
            blenderExecutablePath,
            $"--command extension remove {Quote(packageId)}",
            cancellationToken);
    }
    ...
    private async Task InstallPackageAsync(string blenderExecutablePath, string packagePath, CancellationToken cancellationToken)
    {
        await RunBlenderCommandAsync(
            blenderExecutablePath,
            $"--command extension install-file {Quote(packagePath)} -r {UserDefaultRepoId} -e",
            cancellationToken);
    }
